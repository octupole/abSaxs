...
public:  
  ...
  fftw_plan Plan(Complex *in, Complex *out) {
    const int n[1]={static_cast<const int>(nx)};
...
: fftw(2*nx*ny*(nz/2+1),-1,nx*ny*nz), nx(nx), ny(ny), nz(nz) {Setup(out);}
	...
	/* Added for mpi fftw3 */
	rcfft3d(unsigned int nx, unsigned int ny, unsigned int nz, const Array::array3<double>& in,
			const Array::array3<Complex>& out)
	: fftw(in.Size(),-1,nx*ny*nz), nx(nx), ny(ny), nz(nz) {
		Setup(in,out);
		/* End Added for mpi fftw3 */
	}

#endif  

	virtual fftw_plan Plan(Complex *in, Complex *out) {
		return fftw_plan_dft_r2c_3d(nx,ny,nz,(double *) in,(fftw_complex *) out,
				effort);
	}

	virtual void Execute(Complex *in, Complex *out, bool shift=false) {
...
#ifdef __Array_h__
  /* Added for mpi fftw3 */
  crfft3d(unsigned int nx, unsigned int ny, unsigned int nz, const Array::array3<Complex>& in,
			const Array::array3<double>& out):
		fftw(out.Size(),1,nx*ny*nz), nx(nx), ny(ny), nz(nz) {Setup(in,out);}
  /* End Added for mpi fftw3 */
